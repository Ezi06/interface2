    ...
    function component(object) {
      ...
      this.append = function (element) {
        ...
        item.onFocus = function (elem) {
          ...
          // Add/update title on focus
          if (!cardElement) cardElement = item.render();
          var existingOverlay = cardElement.find('.card-title-overlay');
          var titleText = elem.title || elem.name || '';

          if (titleText && existingOverlay.length === 0) {
            var titleOverlay = $('<div class="card-title-overlay">' + titleText + '</div>');
            cardElement.find('.card__view').first().append(titleOverlay);
          } else if (titleText && existingOverlay.length > 0) {
